  ...
  const fetchFreeExams = async () => {
    try {
      ...
      const { data: examsData, error } = await supabase
        ...
        .eq('classes.section', selectedSystem)
        .limit(5);
  ...
  return (
    <div className="container mx-auto py-8 px-4">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Free Access Notice */}
        {!isFullAccessUser && (
          <div className="mb-8 p-6 bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-950/30 dark:to-orange-950/30 border border-amber-200 dark:border-amber-800 rounded-lg">
            ...
            <p className="text-amber-700 dark:text-amber-300 mb-4">
              {language === 'fr' 
                ? `Vous consultez actuellement ${freeExams.length} épreuves gratuites. Connectez-vous et abonnez-vous pour accéder à toutes les épreuves organisées par classe et matière.` 
                : `You are currently viewing ${freeExams.length} free exam papers. Sign in and subscribe for access to all exams organized by class and subject.`}
            </p>
        ...
        <div className="mb-8">
          <h1 className="text-3xl font-bold mb-2">
            {isFullAccessUser 
              ...
              : (language === 'fr' ? 'Épreuves Gratuites' : 'Free Exam Papers')}
          </h1>
          <p className="text-muted-foreground text-lg">
            {isFullAccessUser
              ...
              : (language === 'fr'
                ? 'Découvrez un aperçu de nos épreuves. Abonnez-vous pour un accès complet.'
                : 'Explore a preview of our exam papers. Subscribe for full access.')}
          </p>
        </div>
              ...
              {freeExams.map((exam, index) => {
                ...
                return (
                  <Card
                    key={exam.id}
                    className={`group cursor-pointer hover:shadow-lg transition-all duration-300 ${colorClasses[colorIndex]}`}
                    onClick={() => navigate(`/exam/${exam.id}?mode=preview`)}
                  >
                    <CardHeader>
